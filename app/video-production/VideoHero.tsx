...
export default function VideoHero() {
    ...
    return (
        ...
        >
            ...
            {/* Carousel Indicators */}
            <div className={`absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex gap-1 transition-opacity duration-500 ${cinemaMode ? "opacity-0" : "opacity-100"}`}>
                {videos.map((_, idx) => (
                    <button
                        key={idx}
                    ...
                    >
                        <span className={`block w-3 h-3 rounded-full transition-all duration-300 ${idx === currentVideoIndex ? "bg-[#FFF4E3] scale-125" : "bg-[#FFF4E3]/40"}`} />
                    </button>
                ))}
            </div>
            ...
            <button
                ...
                className={`absolute bottom-10 left-6 z-20 flex items-center gap-2 px-4 py-2.5 min-h-[44px] rounded-full backdrop-blur-md border transition-all duration-500 group ${cinemaMode ? "bg-[#FFF4E3]/10 border-[#FFF4E3]/30" : "bg-black/40 border-[#FFF4E3]/15 hover:border-[#FFF4E3]/30"}`}
                aria-label={cinemaMode ? "Exit cinema mode" : "Enter cinema mode - tap to view video"}
            >
