...
export const Card = forwardRef<HTMLDivElement, CardProps>(({ customClass, ...rest }, ref) => (
    <div
        ref={ref}
        {...rest}
        className={`absolute top-1/2 left-1/2 rounded-xl border border-white/20 bg-black/40 backdrop-blur-md [transform-style:preserve-3d] [backface-visibility:hidden] ${customClass ?? ''} ${rest.className ?? ''}`.trim()}
    />
));
...
const makeSlot = (i: number, distX: number, distY: number, total: number): Slot => {
    const centerYOffset = ((total - 1) * distY) / 2;
    return {
        x: i * distX,
        y: -i * distY + centerYOffset,
        z: -i * distX * 1.5,
        zIndex: total - i
    };
};
